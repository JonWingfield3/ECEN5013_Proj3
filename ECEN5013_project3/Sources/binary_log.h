...
/******************************************************
 * BinLogStatus BinLogSendData(CircBuf* CB, BinLogID ID)
 *	Description: This function sends all events with a specified BinLogID out via UART
 *		Does nothing is the ID is not found in the BinLog
 *	Parameters:
 *		- CircBuf* CB - reference to CircBuf containing all BinLog items.
 *		- BinLogID ID - ID to search for in buffer.
 *	Possible Return Values:
 *
 ******************************************************/
BinLogStatus BinLogSendData(CircBuf* CB, BinLogID ID);
...
/******************************************************
 * BinLogStatus BinLogEvent(CircBuf* CB, BinLogID ID, uint8_t* payload, uint32_t length)
 *	Description: Adds new BinLog item to CircBuf. Creates BinLog item with call to BinLogCreate() first.
 *	Parameters:
 *		- CircBuf* CB - reference to CircBuf where BinLog is to be added
 *		- BinLogID ID - ID of new BinLog item
 *		- uint8_t* payload - reference to payload
 *		- uint32_t length - size of payload in bytes. Must not exceed BINLOG_MAX_PAYLOAD_SIZE (default is 16)
 *	Possible Return Values:
 *
 ******************************************************/
BinLogStatus BinLogEvent(CircBuf* CB, BinLogID ID, uint8_t* payload, uint32_t length);
...
/******************************************************
 * BinLogStatus BinLogChar(CircBuf* CB, BinLogID ID, uint8_t character)
 *	Description: Creates new BinLog item to indicate specific character has been received.
 *		calls BinLogCreate() to do so.
 *	Parameters:
 *		- CircBuf* CB - reference to CircBuf where BinLog is to be added
 *		- BinLogID ID - ID of new BinLog item
 *		- uint8_t character - character received
 *	Possible Return Values:
 *
 ******************************************************/
BinLogStatus BinLogChar(CircBuf* CB, BinLogID ID, uint8_t character);
